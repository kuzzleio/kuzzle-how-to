...
The [Kuzzle Plugin Engine](/core/1/guides/essentials/plugins) lets you extend Kuzzle's functionality by adding code modules that offer auxiliary features. These modules can:
...
We will create a plugin [listening synchronously](/core/1/plugins/guides/pipes) to Document Controller events in order to report document changes in Cassandra.

### Pipe some events

The first step is to declare which [Plugin Events](/core/1/plugins/guides/events/intro) we are going to pipe. These pipes must be declared in the plugin constructor.
...
Each method will receive two parameters :
 - a [Request](https://github.com/kuzzleio/kuzzle-common-objects#request) object when an event occurs. Depending on the event triggered, the Request exposes a [Response](/core/1/api/essentials/kuzzle-response) object that will contain the result of the controller's action corresponding to the event.
...
Depending on the triggered event, we will have different Response object formats. (Example for the `create` action : [document:create](/core/1/api/controllers/document/create))
(You can refer to the [Document controller documentation](/core/1/api/controllers/document) for the contents of the Response object)
