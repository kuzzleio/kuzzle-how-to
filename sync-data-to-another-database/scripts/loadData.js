...
program
  ...
  .option(
    "--max-count <count>",
    "Max number of documents imported to Kuzzle (rounded to batch size)",
    9999999
  )
  .parse(process.argv);
...
const dataPath = "/yellow_taxi/yellow_taxi_data.csv",
  host = "localhost",
  port = 7512,
...
console.log(
  `Insert maximum of ${program.maxCount} documents in batch of ${program.batchSize}.`
);
    ...
    if (headerSkipped) {
      ...
      documents.push({
        body: {
          ...
          pickup_position: {
            lon: Number.parseFloat(fields[5]),
            lat: Number.parseFloat(fields[6])
          },
          dropoff_position: {
            lon: Number.parseFloat(fields[9]),
            lat: Number.parseFloat(fields[10])
          },
